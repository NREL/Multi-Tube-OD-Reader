...
def test_add_remove_report_ports_to_device():
    ...
    assert device.report_available_ports()==[2,4,5,6,7,8,9,10,14,15]

    assert device.report_ref_ports()==[3,11,12,13]

    device.remove_user("new_run")
    assert device.report_available_ports()==[2,4,5,6,7,8,9,10,12,13,14,15]
    assert device.report_ref_ports()==[3,11]
...
def test_available_test_ports():
    device = Device("Jason", 1323401, 1)
    assert available_test_ports([device])==[tuple((device, N)) for N in range(1,17)]
